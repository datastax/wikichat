...
class CommonPipelineArgs:
    ...
    truncate_first: bool = field(
        default=True,
        metadata={"help": "Truncate the database before starting the pipeline."},
    )

    rotate_collections_every: int = field(
        default=100000,
        metadata={
...
class LoadPipelineArgs(CommonPipelineArgs):
    ...
    file: str = field(
        default="scripts/data/wiki_links.txt",
        metadata={"help": "File of urls, one per line"},
    )
...
class EmbedAndSearchArgs:
    query: str = field(metadata={"help": "String to embedd and using for ANN search"})
    limit: int = field(default=5, metadata={"help": "Limit to use for ANN search."})
...
class SuggestedSearchArgs:
    ...
    delay_secs: float = field(default=2, metadata={"help": "Delay between searches."})
